...
def send_move(sock, mov_command, board_matrix, agent_state, species_dict):
    ...
    if species_dict['our_species'] == 'vampires':
        total_num_moves, source_coordinate, target_coordinate, num_species_to_move, source_move_bool = get_vampires_moves(
            board_matrix, species_dict)
    elif species_dict['our_species'] == 'werewolves':
        total_num_moves, source_coordinate, target_coordinate, num_species_to_move, source_move_bool = get_werewolves_moves(
            board_matrix, species_dict)
...
def start_game():
    ...
    send_nme(sock, 'NME', 'Counter_Team6')
    ...
    if header != "HUM":
        ...
        agent_state['homes_raw'] = receive_data(sock, agent_state['number_of_homes'] * 2,
                                                "{}B".format(agent_state['number_of_homes'] * 2))
    ...
    if header != "MAP":
        ...
        agent_state['map_commands_raw'] = receive_data(sock, agent_state['number_map_commands'] * 5,
                                                       "{}B".format(agent_state['number_map_commands'] * 5))
